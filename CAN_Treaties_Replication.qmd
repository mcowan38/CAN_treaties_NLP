...
-   **CAN_corp** is used for analysis.
    -   **Note:** here is where we thought about aggregating the Douglas Treaties (treatiesDT).
...
-   **Preserve Character of Text**: For string kernels, we retain the original character of the text as much as possible.

    -   This includes maintaining the original casing, punctuation, and possibly even misspellings or idiosyncrasies in the text – elements contributing to the distinctive substrings (consecutive characters) that the kernel will analyze.
...
-   **Retain Word Forms**: Stemming reduces words to their root forms, which can diminish the lexical diversity that a string kernel might exploit to identify subtle textual similarities or differences.
    ...
    -   Avoid stemming unless it's necessary.

### 3. Cleaning Specifics

-   **Remove Non-textual Elements**: Clean the text from non-textual elements (e.g., URLs, email addresses, or any metadata).

-   **Unicode Normalization**: Normalize text to a consistent Unicode format to avoid discrepancies due to encoding variations.
...
-   **Kernel PCA** **(KPCA)** extends the basic idea of Principal Component Analysis (PCA), which is used to reduce the dimensionality of large data sets – by simplifying them into principal components that capture the most important information.
    ...
    1.  **Dealing with Non-Linearity**: Traditional PCA works well with linear relationships, but many real-world data sets – including textual data – exhibit non-linear structures.
        ...
        -   KPCA uses kernel functions to project data into a higher-dimensional space where these non-linear relationships can be linearly separated – potentially uncovering patterns PCA can miss.
    ...
    4.  **Analysis**: The kernel trick allows us to handle text data – typically hard to manage with linear methods – by measuring similarities (or distances) based on the treaties' language and content.
...
plot_ly(
  ...
  layout(
    ...
    title = list(
      text = paste0("<b>Figure 1</b><br><i>Eiganvalues for 10 Components</i>"),
      x = 0.1, 
      y = -1,
      xanchor = "left", 
      yanchor = "top", 
...
-   Clarify if detrended comes from KPCA or from the TDM PC1.
...
### Correlations Tables – aggregated.csv

```{r correlation_tables}
# Load the data
terms_info_aggregated <- read_excel("00_documents/visuals/terms_info_aggregated.xlsx")

# Define specific terms for KPC1 and KPC2 as per user instruction
kpc1_terms <- c("successor", "receipt", "sell", "parcel", "tract", "unto", "grant", 
                "indian", "resid", "protect", "govern", "punish", "surrend", "dominion")

kpc2_terms <- c("father", "children", "great", "white", "thought", "protect", "cultiv", 
                "pursuant", "commission", "hereinafter", "stipul", "offend", "infring", "negoti")

# Filter and create KPC1 table based on selected terms
kpc1_df <- terms_info_aggregated %>%
  filter(Term %in% kpc1_terms) %>%
  select(Term, Frequency = `Frequency...3`, Correlation = `Correlation...4`) %>%
  arrange(desc(Correlation))

# Filter and create KPC2 table based on selected terms
kpc2_df <- terms_info_aggregated %>%
  filter(`Term - KPC2` %in% kpc2_terms) %>%
  select(Term = `Term - KPC2`, Frequency = `Frequency...18`, Correlation = `Correlation...19`) %>%
  arrange(desc(Correlation))

# Display KPC1 table without scroll box
kpc1_df %>%
  kbl(
    caption = "<b>Table 1</b> <br><i>KPC1 Correlations: Land Transactions vs. Governance Agreements</i>",
    col.names = c("Term", "Frequency", "Correlation")
  ) %>%
  kable_classic(full_width = F, html_font = "Times New Roman", lightable_options = "striped") %>%
  footnote(
    general = "Terms in KPC1 reflect transactional language and governance, with high correlations for property rights and low correlations for governance themes."
  )

# Display KPC2 table without scroll box
kpc2_df %>%
  kbl(
    caption = "<b>Table 2</b> <br><i>KPC2 Correlations: Formality and Legal Language</i>",
    col.names = c("Term", "Frequency", "Correlation")
  ) %>%
  kable_classic(full_width = F, html_font = "Times New Roman", lightable_options = "striped") %>%
  footnote(
    general = "Terms in KPC2 highlight the level of formality and hierarchical legal language, contrasting formal legal discourse with more relational language."
  )
...
-   **Complementarity**: Kernel PCA and traditional PCA serve complementary roles. Kernel PCA can capture nonlinear relationships in high-dimensional data, making it suitable for complex text data structures – though the results might be more abstract and harder to interpret directly in terms of original features (words). On the other hand, traditional PCA with a TDM provides a more interpretable model, as it directly relates to the terms used across the documents (Spirling 2012).
...
plot_ly(data = importance_df_pc1[1:35,], x = ~Importance, y = ~Term, type = 'bar',
  ...
  layout(
    title = list(
      text = paste0("<b>Figure 3</b><br><i>KPC1 Importance: Top 35 Terms</i>"),
      x = 0.1, 
      y = -1,
      xanchor = "left", 
      yanchor = "top", 
...
plot_ly(data =  top_terms_pc2[1:35,], x = ~Importance, y = ~Term, type = 'bar',
  ...
  layout(
    title = list(
      text = paste0("<b>Figure 4</b><br><i>KPC2 Importance: Top 35 Terms</i>"),
      x = 0.1, 
      y = -1,
      xanchor = "left", 
      yanchor = "top", 
...
# Create and display a compact interactive biplot
biplot <- plot_ly(
  data = pca_merged_df,
  x = ~PC1_Score,
  y = ~PC2_Score,
  ...
  marker = list(size = 7, opacity = 0.7, line = list(width = 0.8, color = '#FFFFFF')),
  text = ~paste(
  ...
  layout(
    title = list(
      text = "<b>Figure 2</b><br><i>Biplot of Canadian-Indigenous Treaties</i>",
      x = 0.1, 
      y = -1,
    ...
    xaxis = list(
      title = "<b>Principal Component 1 (Topic)</b>", 
      titlefont = list(size = 12), 
      tickfont = list(size = 10)
    ),
    yaxis = list(
      title = "<b>Principal Component 2 (Formality)</b>", 
      titlefont = list(size = 12), 
      tickfont = list(size = 10)
    ),
    legend = list(
      title = list(text = "<b>Treaty Type</b>", font = list(size = 12)), 
      font = list(size = 10), 
      orientation = 'h', # horizontal layout
      x = 0.5, xanchor = "center", y = -0.3  # position it below the plot
    ),
    margin = list(l = 80, r = 80, t = 100, b = 120)
  )

biplot
# Save the compact interactive biplot as an HTML file
...
### **Spirling (2012) Comparison Table:**

```{r}
# Comparing Canadian (KPC1 & KPC2) vs. U.S. treaties
treaty_data <- data.frame(
  Stem = c("friendship", "peace", "mutual", "boundary", "relinquish", "dollar", "tract", "land", "reservation"),
  US_Correlation = c(0.62, 0.56, 0.49, -0.01, -0.08, -0.10, -0.20, -0.49, -0.49),  # Spirling's U.S. data
  Canadian_KPC1 = c(-0.51, 0.32, 0.03, -0.04, -0.27, -0.32, 0.52, 0.11, -0.32),   # Example Canadian KPC1 values
  Canadian_KPC2 = c(0.37, 0.04, 0.14, -0.10, 0.15, -0.49, -0.27, 0.03, -0.50)    # Example Canadian KPC2 values
)

# Create the table with kableExtra
treaty_data %>%
  kbl(
    caption = "<b>Table 3</b> <br><i>Comparison of Stem Correlation Scores for U.S. and Canadian Treaties</i>",
    col.names = c("Word", "U.S. Treaties", "Canadian Treaties (KPC1)", "Canadian Treaties (KPC2)"),
    align = c("l", "c", "c", "c")
  ) %>%
  kable_classic(full_width = F, html_font = "Times New Roman") %>%
  add_header_above(c(" " = 1, "Correlation" = 3)) %>%
footnote(
  general = "Correlation scores for selected stems in U.S. treaties (Spirling, 2012) and Canadian treaties (KPC1 and KPC2). Spirling's analysis identified patterns of tonal shifts in U.S. treaties, where positive and negative correlations reflected changes in relational tone over time. In contrast, the Canadian treaty correlations are inconsistent across KPC1 and KPC2, showing no unified pattern that would indicate tonal shifts. This suggests a lack of relational or conflict-driven language dynamics in Canadian treaties, reflecting instead a stable, administrative language that does not respond to historical context in the same way."
)
