...
treatiesACUT <-
  VCorpus(
    DirSource(
      "./00_data/treaties_USA/justdocsACUT/"
    ),
    readerControl = list(
      reader = readPlain,
      language = "en",
...
treatiesRCUT <-
  VCorpus(
    DirSource(
      "./00_data/treaties_USA/justdocsRCUT/"
    ),
    readerControl = list(
      reader = readPlain,
      language = "en",
...
treatiesUCUT <-
  VCorpus(
    DirSource(
      "./00_data/treaties_USA/justdocsUCUT/"
    ),
    readerControl = list(
      reader = readPlain,
      language = "en",
...
treatiesVCUT <-
  VCorpus(
    DirSource(
      "./00_data/treaties_USA/justdocsVCUT/"
    ),
    readerControl = list(
      reader = readPlain,
      language = "en",
...
-   Here, **FULL_corp** becomes our original, unaltered corpus database for search functionality.
-   **US_corp** is used for analysis.
    -   **Note:** here is where we can think about removing treaty-sets collectively.
...
### US_corp – Pre-processing (for analysis)

```{r}
# [Optional] More cleaning/transformation:
US_corp <-
  tm_map(FULL_corp, content_transformer(tolower)) # Convert to lower case
US_corp <-
  tm_map(US_corp, removePunctuation) # Remove punctuation
US_corp <-
  tm_map(US_corp, removeWords, stopwords("en")) # Remove English stop words
US_corp <- tm_map(US_corp, stemDocument) # Stem the words
...
-   **Preserve Character of Text**: For string kernels, it's beneficial to retain the original character of the text as much as possible.
    ...
    -   These elements can contribute to distinctive substrings (consecutive characters) that the algorithm will analyze.
...
# Output the result:
save(US_KPC1, file = "./00_scripts/01_US_modified/US_KPC1.rdata")
save(US_KPC2, file = "./00_scripts/01_US_modified/US_KPC2.rdata")
save(US_KPC10, file = "./00_scripts/01_US_modified/US_KPC10.rdata")

# Load Data:
load("./00_scripts/01_US_modified/US_KPC1.rdata")
load("./00_scripts/01_US_modified/US_KPC2.rdata")
load("./00_scripts/01_US_modified/US_KPC10.rdata")
```
...
```{r}
# Plotting the eigenvalues (10 features):
...
pc1_scores <-
  US_KPC1@rotated[, 1] # Adjusting this line to correctly access the PCA
...
pc2_scores <- US_KPC2@rotated[, 2] # Adjusting this line to correctly access the PCA
...
write.csv(
  results_df,
  "./00_US_documents/visuals/US_correlations_PC1.csv",
  row.names = FALSE
)
```
...
write.csv(
  results_df_pc2,
  "./00_US_documents/visuals/US_correlations_PC2.csv",
  row.names = FALSE
)
```
...
write.csv(
  importance_df_pc1,
  "./00_US_documents/visuals/US_term_importance_PC1.csv",
  row.names = FALSE
)
```
...
write.csv(
  importance_df_1,
  "./00_US_documents/visuals/US_term_importance_xgboost_PC1.csv",
  row.names = FALSE
)
```
...
rf_model_pc2 <- randomForest(as.matrix(dtm_1),
                             pc2_scores,
                             ntree = 2000,
                             importance = TRUE)
...
write.csv(
  importance_df_pc2,
  "./00_US_documents/visuals/US_term_importance_PC2.csv",
  row.names = FALSE
)
```
...
write.csv(
importance_df_xgb_2,
  "./00_US_documents/visuals/US_term_importance_xgboost_PC2.csv",
  row.names = FALSE
)
```
...
top_terms_pc2$Term <- factor(top_terms_pc2$Term,
                         levels = unique(top_terms_pc2$Term)[order(top_terms_pc2$Importance, decreasing = F)])
...
search_term_in_corpus <- function(doc_texts, search_term) {
  results <- lapply(doc_texts, function(doc) {
...
found_sentences <-
  search_term_in_corpus(doc_texts = search_texts, search_term = "Indian")

# Output found sentences
# print(found_sentences)
```

# [US_timetrend.R]{.underline}

-   UniverseCases.csv needed to match the number of elements/entries as were in the KPCA.
...
universe <- read.csv("./00_documents/01_US_spreadsheets/UniverseCases.csv")
universed <- gsub("\\/", "-", universe$Date)
universe$Date <-  as.Date(universed, format = "%Y-%m-%d")


# Pull in PC1 estimates
load(file = "./00_scripts/01_US/US_KPC1.rdata")
rot <- US_KPC1@rotated
...
load(file = "./00_scripts/01_US/US_KPC2.rdata")
rot <- US_KPC2@rotated
